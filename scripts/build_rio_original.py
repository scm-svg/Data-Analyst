#!/usr/bin/env python3
"""Build RIO ORIGINAL dashboard + proyecciones Excel from sales/inventory files."""
...
TEMPLATE = UPLOADS / "index_621a.html"
VENTAS_XLSX = UPLOADS / "RIO_VENTAS_ACTUALIZADS_50ed.xlsx"
INV_XLSX = UPLOADS / "RIO_INVENTARIO_ACTUALIZADO_cae7.xlsx"
OUT_HTML = ROOT / "DASHBOARD_RIO_ORIGINAL.html"
OUT_XLSX = ROOT / "RIO_ORIGINAL_Proyeccion_Produccion.xlsx"
MODELO = "RIO ORIGINAL"
...
# Colores disponibles para producción — catálogo Rio Original (mismos criterios que Mar)
COLORES_PRODUCCION = {
    "CAB": [
...
MAX_RANGE_PCT = 0.06
TELA_CONSUMO = {"CAB": 0.66, "DAMA": 0.52, "KIDS": 0.32}
TELA_UNIDAD = "kg"
TELA_SS = 0.20
...
def active_colors(genero: str, mix_vel: pd.DataFrame, all_sales: pd.DataFrame) -> list[str]:
    """Only production-catalog colors, ordered by sales volume in mix_vel."""
...
def build_gender_plan_proportional(
    ...
    """Build production: mix ventas históricas (color) + Jun–Ago (talla), neto de stock."""
    ...
    mix_vel = hist_vel
    talla_vel = hist_vel[hist_vel["mes"].isin(VELOCITY_PERIODS)]
    colors = active_colors(genero, mix_vel, all_sales)
    sold_tallas = active_tallas(genero, talla_vel, for_production=True)
    excluded_tallas = PRODUCTION_EXCLUDE_TALLAS.get(genero, set())
    color_sales = mix_vel.groupby("color")["v"].sum().sort_values(ascending=False)
    color_total = color_sales.sum() or 1
    gender_total = mix_vel["v"].sum() or 1
    talla_total = talla_vel["v"].sum() or 1
    gender_talla = talla_vel.groupby("talla")["v"].sum()
    gender_talla_pct = (gender_talla / talla_total).to_dict()
    ...
    for color in colors:
        ...
        c_hist = mix_vel[mix_vel["color"] == color]
        c_talla_hist = c_hist.groupby("talla")["v"].sum()
        c_talla_recent = talla_vel[talla_vel["color"] == color].groupby("talla")["v"].sum()
        c_total = float(c_hist["v"].sum())
        c_talla = c_talla_recent if c_talla_recent.sum() > 0 else c_talla_hist
        ...
        for talla in color_tallas:
            ...
            ct_hist = float(c_talla_hist.get(talla, 0))
            sales_share = (ct_hist / gender_total) if ct_hist > 0 else 0.0
            if ct > 0:
                t_share = ct / c_talla.sum() if c_talla.sum() else gender_talla_pct.get(talla, 0)
            elif c_total > 0 and talla in sold_tallas:
                t_share = gender_talla_pct.get(talla, 0)
            else:
                t_share = gender_talla_pct.get(talla, 0)
                sales_share = sales_share or (t_share / max(len(colors), 1) * 0.25)

            if sales_share <= 0 and t_share <= 0:
                continue

            suggested = sales_share * target
            ...
            w = sales_share * production_talla_boost(genero, talla)
            if w <= 0:
                continue
            ...
            meta[(color, talla)] = {
                ...
                "net_after_stock": round(max(0.0, suggested - t_stk), 1),
...
def build_data(sales: pd.DataFrame, inv: pd.DataFrame) -> dict:
    ...
    for _, r in inv.iterrows():
        key_plain = f"{r['color']}/{r['talla']}/{r['genero']}"
        key_model = f"{MODELO}/{r['genero']}/{r['color']}/{r['talla']}"
        stock[key_model] += r["v"]
        stock[key_plain] += r["v"]
        stock_by_store[r["tienda"]][key_plain] += r["v"]
        stock_by_modelo[MODELO] += r["v"]
    ...
    all_stores = ["SAMBIL", "GRIE", "CERRO VERDE", "CHACAO", "GRAND", "TOLON", "VELA", "BARQUISIMETO", "WEB"]
    ...
    return {
        ...
        "raw_rows": raw_rows,
        "stock": stock_dict,
        "stock_total": int(sum(v for k, v in stock_dict.items() if k.startswith(MODELO + "/"))),
        "stock_by_modelo": {MODELO: int(stock_by_modelo[MODELO])},
        "stock_by_store": sbs,
...
def export_excel(data: dict, path: Path) -> None:
    ...
    with pd.ExcelWriter(path, engine="xlsxwriter") as writer:
        ...
        row = 0
        ws.merge_range(row, 0, row, 5, "RIO ORIGINAL — PROYECCIÓN DE PRODUCCIÓN · CAB / DAMA / KIDS", title)
        row += 1
        ws.merge_range(
            row, 0, row, 5,
        ...
        bullets = [
            ...
            "• Solo colores ACTIVOS para producción según catálogo Rio Original.",
            "• Color: ventas históricas Oct–Ago · Talla: Jun–Ago · Stock actual referenciado en Resumen y dashboard.",
            f"• Tela {TELA_NOMBRE}: consumo en {TELA_UNIDAD}/und (CAB {TELA_CONSUMO['CAB']} · DAMA {TELA_CONSUMO['DAMA']} · KIDS {TELA_CONSUMO['KIDS']}).",
            ...
            "• KIDS: sin talla 1 en producción.",
        ]
        for b in bullets:
        ...
        row = 0
        ws.write(row, 0, "RIO ORIGINAL — CANTIDADES POR TALLA (MÍN / MÁX)", title)
        row += 2
        for genero in ["CAB", "DAMA", "KIDS"]:
            rd = rango[genero]
            s = summary[genero]
        ...
        row = 0
        ws.write(row, 0, "RIO ORIGINAL — CANTIDADES POR COLOR (colores activos para producción)", title)
        row += 2
        for genero in ["CAB", "DAMA", "KIDS"]:
            rd = rango[genero]
            s = summary[genero]
        ...
        row = 0
        ws.write(row, 0, "RIO ORIGINAL — PRODUCCIÓN POR COLOR Y TALLA", title)
        row += 2
        for genero in ["CAB", "DAMA", "KIDS"]:
            rd = rango[genero]
            g_label = GENDER_XL[genero]
        ...
        row = 0
        ws.write(row, 0, f"RIO ORIGINAL — COMPRA DE TELA {TELA_NOMBRE.upper()}", title)
        row += 1
        ws.write(row, 0, f"Consumo promedio por unidad ({TELA_UNIDAD}/und) y stock de seguridad", wrap)
        row += 2
        ws.write(row, 0, "Stock de seguridad tela (compra)", bold)
        ws.write(row, 1, TELA_SS, pct)
        row += 2
        ws.write(row, 0, "Género", bold)
        ws.write(row, 1, f"Consumo ({TELA_UNIDAD}/und)", bold)
        row += 1
        for g in ["CAB", "DAMA", "KIDS"]:
            ws.write_row(row, 0, [GENDER_XL[g], TELA_CONSUMO[g]])
            row += 1
        row += 1
        tela_hdr = ["Color", "Und Mín", "Und Máx", "Kg consumo Mín", "Kg consumo Máx", "Kg compra Mín (+SS)", "Kg compra Máx (+SS)"]
        for genero in ["CAB", "DAMA", "KIDS"]:
            cons = TELA_CONSUMO[genero]
            g_label = GENDER_XL[genero]
            ws.write(row, 0, f"{g_label} — COMPRA DE TELA POR COLOR (consumo {cons} {TELA_UNIDAD}/und)", section)
            row += 1
        ...
        row = 0
        ws.write(row, 0, "RIO ORIGINAL — DISTRIBUCIÓN POR TIENDA", title)
        row += 2
        for genero in ["CAB", "DAMA", "KIDS"]:
            gc = ctx["generos"][genero]
            g_label = GENDER_XL[genero]
            prod_min = summary[genero]["produce"]
            prod_max = summary[genero]["produce_max"]
            ws.write(row, 0, f"{g_label} — DISTRIBUCIÓN POR TIENDA (según peso de velocidad)", section)
            row += 1
            ws.write_row(row, 0, ["Tienda", "Peso %", "Mínimo", "Máximo"], hdr)
            row += 1
            t_min = t_max = 0
            for sr in gc["store_rows"]:
        ...
        row = 0
        ws.write(row, 0, "METODOLOGÍA — RIO ORIGINAL · PROYECCIÓN DE PRODUCCIÓN (CAB / DAMA / KIDS)", title)
        row += 2
        metodologia = [
            ...
            (f"• Colores activos para producción: catálogo Rio Original ({len(COLORES_PRODUCCION['CAB'])} CAB · {len(COLORES_PRODUCCION['DAMA'])} DAMA · {len(COLORES_PRODUCCION['KIDS'])} KIDS).", None),
            (f"• Consumo tela {TELA_NOMBRE}: CAB {TELA_CONSUMO['CAB']} kg/und · DAMA {TELA_CONSUMO['DAMA']} kg/und · KIDS {TELA_CONSUMO['KIDS']} kg/und.", None),
            ...
            ("• Color: % sobre ventas históricas Oct–Ago; talla: % sobre ventas Jun–Ago.", None),
            ("• Producción proporcional a ventas; stock actual para cobertura (ver dashboard Decisiones).", None),
            ("• CAB: color 'Gris Claro' (agrupa Gris + Gris Claro). Sin 3XL/4XL (CAB) ni 2XL (DAMA).", None),
            ("• CAB: sin 3XL/4XL · boost 2XL +22%. DAMA: sin 2XL · boost L +22%. KIDS: sin talla 1.", None),
            ("5. CURVAS DE TALLA, COLOR Y TELA", None),
            ("• Producción Color × Talla: matriz color × talla (Mín/Máx por color).", None),
            (f"• Compra de tela: kg = unidades × consumo género × (1 + {int(TELA_SS * 100)}% SS).", None),
...
def patch_html(template: str, data: dict) -> str:
    ...
    html = re.sub(r"var DATA=\{.*?\};", f"var DATA={data_json};", template, count=1, flags=re.DOTALL)
    if "var DATA=" + data_json not in html:
...
"""
    html = html.replace('<div class="sec" id="sec-decisiones">', inv_section + '<div class="sec" id="sec-decisiones">')

    html = html.replace("CAB vs DAMA", "CAB / DAMA / KIDS")
    html = html.replace(
    ...
    html = html.replace(
        ...
        '<div class="sub">Distribución sugerida · <span style="color:#f97316">🆕 VELA (1× GRIETA) · BARQUISIMETO prom(G+Ch+T) · WEB 50% líder</span> · TOLON ×1.45</div>',
    )
    ...
    if "var COLORES_DISP=" not in html:
        html = html.replace(
            "var GICO={CAB:'👔',DAMA:'👗',KIDS:'👶'};",
            "var GICO={CAB:'👔',DAMA:'👗',KIDS:'👶'};\n"
            "var COLORES_DISP={CAB:{Aguamarina:1,'Amarillo Neón':1,'Azul Lavanda':1,'Azul Marino':1,'Azul Rey':1,Blanco:1,'Gris Claro':1,Negro:1,Rojo:1,'Verde Militar':1,Vinotinto:1},"
    ...
    store_labels = (
        "var STORE_LABELS={all:'📊 Producción Total',SAMBIL:'Sambil',GRIE:'Grieta',"
        "'CERRO VERDE':'C.Verde',CHACAO:'Chacao',GRAND:'Grand',TOLON:'Tolón',"
        "VELA:'Vela ★',BARQUISIMETO:'Barquisimeto ★',WEB:'Web ★',TALLER:'Taller'};"
    )
    if "var STORE_LABELS=" in html:
        html = re.sub(r"var STORE_LABELS=\{.*?\};", store_labels, html, count=1)
    else:
        html = html.replace("var ALL_STORES=DATA.all_stores;", store_labels + "\nvar ALL_STORES=DATA.all_stores;")

    html = html.replace(
        "var ALL_STORES=DATA.all_stores;",
        "var ALL_STORES=['SAMBIL','GRIE','CERRO VERDE','CHACAO','GRAND','TOLON','VELA','BARQUISIMETO','WEB'];",
    )
    html = html.replace("var NEW_STORES=['MARGARITA'];", "var NEW_STORES=['VELA','BARQUISIMETO','WEB'];")
    html = html.replace(
        "var NEW_STORE_CAPS={'MARGARITA':{base:'GRIETA',mult:1.5,label:'1.5\\xd7 capacidad GRIETA'}};",
        "var NEW_STORE_CAPS={VELA:{base:'GRIE',mult:1,label:'1× capacidad GRIETA'},"
        "BARQUISIMETO:{base:'GRIE',mult:1,label:'Prom(Grieta+Chacao+Tolón)'},"
        "WEB:{base:'SAMBIL',mult:0.5,label:'50% tienda líder'}};",
    )
...
function getStoreWeights(rows,l){
  ...
  ['SAMBIL','GRIE','CERRO VERDE','CHACAO','GRAND','TOLON'].forEach(function(s){
    var base=byT[s]||0;if(s==='TOLON')base=Math.round(base*tb);w[s]=base;
  });
...
"""
    if "function getStoreWeights" not in html:
        html = html.replace("function getSW(rows,genero,modelo,meses){", get_sw + "\nfunction getSW(rows,genero,modelo,meses){")
    ...
    html = html.replace(
        ...
        "function getNewStoreShare(store,realShares){\n  if(store==='VELA')return realShares['GRIE']||0;\n"
        "  if(store==='BARQUISIMETO')return ((realShares['GRIE']||0)+(realShares['CHACAO']||0)+(realShares['TOLON']||0))/3;\n"
        "  if(store==='WEB'){var mx=Math.max(realShares['SAMBIL']||0,realShares['GRIE']||0,realShares['CERRO VERDE']||0,realShares['CHACAO']||0,realShares['GRAND']||0,1);return mx*0.5;}\n"
        "  var cap=NEW_STORE_CAPS[store];if(!cap)return 0;\n  return(realShares[cap.base]||0)*cap.mult;\n}",
    )

    html = html.replace("var isNew=NEW_STORES.indexOf(store)>=0;", "var isNew=store==='VELA'||store==='BARQUISIMETO'||store==='WEB';")
    html = html.replace("  var storeList=ALL_STORES.concat(NEW_STORES);", "  var storeList=ALL_STORES;")
    html = html.replace(
        "'+store+'</div>'+(isNew&&cap?'<div style=\"font-size:0.6rem;color:#f97316\">'+cap.label+'</div>':'')+'</div>'",
        "'+(STORE_LABELS[store]||store)+'</div>'+(isNew&&cap?'<div style=\"font-size:0.6rem;color:#f97316\">'+cap.label+'</div>':'')+'</div>'",
    )
...
function rInventario(){
  ...
  order.forEach(function(store){
    var isTaller=store==='TALLER',data=sbs[store]||{};
    var cM={};Object.entries(data).forEach(function(kv){var p=kv[0].split('/'),col=p[0];cM[col]=(cM[col]||0)+kv[1];});
    var colorItems=Object.entries(cM).sort(function(a,b){return b[1]-a[1];}).map(function(ce){
      var col=ce[0],colV=ce[1],tM={};
      Object.entries(data).forEach(function(kv){var p=kv[0].split('/');if(p[0]===col)tM[p[1]+' · '+p[2]]=(tM[p[1]+' · '+p[2]]||0)+kv[1];});
      var tRows=Object.entries(tM).sort(function(a,b){return b[1]-a[1];}).map(function(tv){
        return'<div style="display:flex;gap:6px;padding:2px 0;font-size:0.68rem"><div style="width:70px;font-weight:700;color:#c0c0e8">'+tv[0]+'</div><div style="color:var(--mu)">'+tv[1]+' und</div></div>';
      }).join('');
    ...
    h+='<div style="background:'+(isTaller?'rgba(249,115,22,.08)':'var(--s2)')+';border:1px solid '+(isTaller?'#f9731633':'var(--brd)')+';border-radius:12px;margin-bottom:10px">'+
      '<div onclick="toggleInv(\\''+sUid+'\\')" style="display:flex;align-items:center;gap:10px;padding:14px 16px;cursor:pointer">'+
      '<span id="arr_'+sUid+'" style="color:var(--ac);font-size:0.7rem">▶</span>'+
      '<div style="flex:1"><div style="font-family:var(--fh);font-weight:800;font-size:0.85rem">'+(isTaller?'🏭':'🏪')+' '+(STORE_LABELS[store]||store)+'</div></div>'+
      '<div style="font-family:var(--fh);font-weight:800;color:'+(isTaller?'#f97316':'var(--yw)')+';font-size:1.1rem">'+storeTotals[store]+'</div></div>'+
      '<div id="'+sUid+'" style="display:none;padding:0 16px 14px">'+(colorItems||'<div class="nodata">Sin stock</div>')+'</div></div>';
  });
  document.getElementById('invByStore').innerHTML=h;
}
"""
        html = html.replace("function setDecMeses(n){", inv_js + "\nfunction setDecMeses(n){")
