 ...
 *     cuando TODOS los SKUs del desglose lo están. El encabezado
 *     Plan 12 sem = plan del horizonte; Pendiente = lo que quedó
 *     fuera; A producir = Faltante de Por Hacer. El gráfico
 *     Planificado vs producido usa Cantidad Solicitada y Cantida
 *     Producida del archivo.
 ...
 *     línea libre, secuencian ahí por color (Negro → Blanco → Marino →
 ...
 *     Si la celda viene vacía, se usa el respaldo L1-4=130 / L5=40.
 *   - SECUENCIA DE GÉNERO: ver LOTES POR COLOR Y GÉNERO (arriba).
 ...
 *   - LÍNEA 5: trabaja 2 modelos en paralelo (rueda de 5 pzas
 *     cuando hay dos), salvo que sean el mismo producto en distinto
 *     género: esos van en secuencia. Si solo hay un modelo, usa su
 *     Cap Produccion por Dia (típico 40).
 *   - URGENTE (después de Especial y de la cantidad mínima), luego la
 *     fecha más próxima. Un modelo Urgente con 2+ líneas usa ambas.
 *     Líneas 1-4 = un modelo a la vez, salvo uno o dos modelos a
 *     media estación elegidos al generar. L5 hasta 2 en paralelo.
...
function nombresHojasSemanas_() {
  return NOMBRES_HOJAS_SEMANAS.slice();
}
...
function onOpen() {
  ...
  ui.createMenu("👁️ Ver Pestañas")
    ...
    .addItem("📅 Semana 6", "mostrarSemana6")
    .addItem("📅 Semana 7", "mostrarSemana7")
...
function mostrarSemana6() { mostrarHoja_("Semana 6"); }
function mostrarSemana7() { mostrarHoja_("Semana 7"); }
...
function ocultarPestanasSemanales() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var hojas = nombresHojasSemanas_().concat([
...
function filtrarVistas() {
  ...
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var hojasSemanales = nombresHojasSemanas_();
  ...
  for (var i = 0; i < datos.length; i++) {
    for (var j = 0; j < datos[i].length - 1; j++) {
      ...
      if (etiqueta.indexOf("semanas") !== -1) {
        ...
        if (n >= 1 && n <= SEMANAS_MAX) cfg.semanas = n;
      }
...
function esTokenGenero_(s) {
  var g = quitarTildes_(normLow_(s));
  if (g === "" || g === "--") return false;
  return g === "cab" || g.indexOf("caballer") === 0 || g === "hombre" || g === "male" ||
    g === "dama" || g === "mujer" || g === "female" || g === "girl" ||
    g === "kids" || g === "nino" || g === "nina" || g === "ninos" || g === "ninas" ||
    g === "infantil" || g === "nino/nina" || g === "unisex" || g === "boy";
}

function ordenGenero_(g) {
  var s = quitarTildes_(normLow_(g));
  if (s === "cab" || s.indexOf("caballer") === 0 || s === "hombre" || s === "male" || s === "boy") return 0;
  if (s === "dama" || s === "mujer" || s === "female" || s === "girl") return 1;
  if (s === "kids" || s === "nino" || s === "nina" || s === "ninos" || s === "ninas" || s === "infantil") return 2;
  if (s === "" || s === "--") return 3;
  return 4;
}
...
function familiaDeTarea_(t) {
  if (t && t.familia) return t.familia;
  return familiaDeNombre_(t ? t.modelo : "");
}
...
function capFallbackLinea_(lin) {
  return CAP_POR_LINEA[String(lin)] || 130;
}

function capDeTarea_(t, lin) {
  var c = t ? Number(t.cap) : 0;
  if (c > 0) return c;
  var l = lin || (t && t.lineaFija) || (t && t.lineas && t.lineas[0]) || "1";
...
function debeArchivarMo_(tipo, status) {
  var st = quitarTildes_(normLow_(status));
  var tp = quitarTildes_(normLow_(tipo));
  if (st === "cancelada") return true;
  if (st === "hecho" && tp.indexOf("especial") !== -1) return false;
  if (st === "hecho") return true;
  return false;
}

function numEnCeldaDia_(v) {
  if (v === "--" || v === "" || v === null || v === undefined) return 0;
  return Number(v) || 0;
}

function primerDiaEnRango_(row, diaIni, diaFin) {
  for (var i = diaIni; i <= diaFin; i++) {
    if (numEnCeldaDia_(row[i]) > 0) return i;
  }
  return 99;
}

function filaContinuaDespues_(row, primerDia, diaIni, diaFin) {
  if (primerDia >= 99) return false;
  for (var i = primerDia + 1; i <= diaFin; i++) {
    if (numEnCeldaDia_(row[i]) > 0) return true;
  }
  return false;
}

function cmpFilasFlujoTablero_(a, b, diaIni, diaFin, idxNombre) {
  var da = primerDiaEnRango_(a, diaIni, diaFin);
  var db = primerDiaEnRango_(b, diaIni, diaFin);
  if (da !== db) return da - db;
  var ca = filaContinuaDespues_(a, da, diaIni, diaFin) ? 1 : 0;
  var cb = filaContinuaDespues_(b, db, diaIni, diaFin) ? 1 : 0;
  if (ca !== cb) return ca - cb;
  var qa = numEnCeldaDia_(a[da]);
  var qb = numEnCeldaDia_(b[db]);
  if (qa !== qb) return qa - qb;
  var na = idxNombre !== undefined ? a[idxNombre] : a[1];
  var nb = idxNombre !== undefined ? b[idxNombre] : b[1];
...
function cloneTask(t, newQty, isFase2) {
  return {
    ...
    mo: t.mo,
    genero: t.genero,
    familia: t.familia || familiaDeTarea_(t),
    color: t.color,
...
function esLineaEstandar_(lin) {
  var n = String(lin);
  return n === "1" || n === "2" || n === "3" || n === "4";
}

function modeloUsaLineaEstandar_(m) {
  if (!m || !m.tareas) return false;
  var i, j, ls;
  for (i = 0; i < m.tareas.length; i++) {
    ls = m.tareas[i].lineas || [];
    for (j = 0; j < ls.length; j++) {
      if (esLineaEstandar_(ls[j])) return true;
    }
  }
  return false;
}

function parsearFraccionParcial_(txt) {
  var s = String(txt === null || txt === undefined ? "" : txt).trim().replace("%", "").replace(",", ".");
  if (s === "") return 0;
  var n = Number(s);
...
function resolverNombreModeloParcial_(texto, candidatos) {
  var raw = norm_(texto);
  if (raw === "" || !candidatos || !candidatos.length) return null;
  var entero = Math.floor(Number(raw));
  if (String(entero) === raw && entero >= 1 && entero <= candidatos.length) {
    return candidatos[entero - 1];
  }
  var alvo = claveModeloNorm_(raw);
  var i, m, hits;
  for (i = 0; i < candidatos.length; i++) {
    if (claveModeloNorm_(candidatos[i].nombre) === alvo) return candidatos[i];
  }
  var alvoSin = claveModeloNorm_(modeloSinLote_(raw));
  hits = [];
  for (i = 0; i < candidatos.length; i++) {
    m = candidatos[i];
    if (claveModeloNorm_(modeloSinLote_(m.nombre)) === alvoSin) hits.push(m);
  }
  if (hits.length === 1) return hits[0];
  hits = [];
  for (i = 0; i < candidatos.length; i++) {
    m = candidatos[i];
    if (claveModeloNorm_(m.nombre).indexOf(alvo) !== -1) hits.push(m);
  }
  if (hits.length === 1) return hits[0];
  return null;
}

/** Parte "1, 3" / "1 3" / "RIO DAMA, VITA BIKER" / "1 y 3" en textos sueltos. */
function partirTextosModeloParcial_(texto) {
  var raw = norm_(texto);
  if (raw === "") return [];
  if (/^\d+(\s+\d+)+$/.test(raw)) return raw.split(/\s+/);
  return raw.split(/[,;\/|]+|\s+y\s+|\s+and\s+/i).map(function (s) {
    return s.trim();
  }).filter(function (s) { return s !== ""; });
}

function resolverNombresModeloParcial_(texto, candidatos) {
  var partes = partirTextosModeloParcial_(texto);
  if (!partes.length) return [];
  var out = [];
  var seen = {};
  var i, m, k;
  for (i = 0; i < partes.length; i++) {
    m = resolverNombreModeloParcial_(partes[i], candidatos);
    if (!m) return null;
    k = claveModeloNorm_(m.nombre);
    if (seen[k]) continue;
    seen[k] = true;
    out.push(m);
  }
...
function armarCfgModeloParcial_(elegidos, fracc) {
  var a = elegidos[0];
  var b = elegidos.length > 1 ? elegidos[1] : null;
  return {
    activo: true,
    nombre: a.nombre,
    clave: claveModeloNorm_(a.nombre),
    nombres: elegidos.map(function (m) { return m.nombre; }),
    claves: elegidos.map(function (m) { return claveModeloNorm_(m.nombre); }),
    companero: b ? b.nombre : "",
    claveCompanero: b ? claveModeloNorm_(b.nombre) : "",
    fraccion: fracc || FRACCION_PARCIAL_DEFAULT
  };
}

function nombresCfgModeloParcial_(cfg) {
  if (!cfg) return [];
  var noms = (cfg.nombres && cfg.nombres.length) ? cfg.nombres.slice() : [];
  if (!noms.length && cfg.nombre) noms.push(cfg.nombre);
  if (cfg.companero && noms.indexOf(cfg.companero) === -1) noms.push(cfg.companero);
  return noms;
}

function coincideModeloParcialNombre_(m, nombre, clave) {
  if (!m || (!nombre && !clave)) return false;
  var alvo = clave || claveModeloNorm_(nombre);
  if (!alvo) return false;
  if (claveModeloNorm_(m.nombre) === alvo) return true;
  if (claveModeloNorm_(modeloSinLote_(m.nombre)) === alvo) return true;
  if (nombre && claveModeloNorm_(modeloSinLote_(m.nombre)) === claveModeloNorm_(modeloSinLote_(nombre))) {
    return true;
  }
  return false;
}

function textoModeloParcial_(cfg) {
...
function preguntarModeloParcial_(listaModelos) {
  var off = modeloParcialOff_();
  try {
    var ui = SpreadsheetApp.getUi();
    var cands = (listaModelos || []).filter(modeloUsaLineaEstandar_);
    ...
    if (r1 !== ui.Button.YES) return off;
    var maxLista = 25;
    var lista = cands.slice(0, maxLista).map(function (m, i) {
      return (i + 1) + ") " + m.nombre;
    }).join("\n");
    if (cands.length > maxLista) {
      lista += "\n... y " + (cands.length - maxLista) + " más. Puedes escribir el nombre.";
    }
    var r2 = ui.prompt(
      "Modelos a media estación (1 o 2)",
      "Escribe UNO o DOS modelos (número o nombre), separados por coma.\n" +
      "Ejemplos: 1, 3   o   RIO DAMA, VITA BIKER DAMA\n\n" + lista,
      ui.ButtonSet.OK_CANCEL
    );
    if (r2.getSelectedButton() !== ui.Button.OK) return off;
    var elegidos = resolverNombresModeloParcial_(r2.getResponseText(), cands);
    if (!elegidos || !elegidos.length) {
      ui.alert("No se reconoció el modelo. Se genera el plan sin paralelo extra en L1-4.");
      return off;
    }
    ...
    var fracc = FRACCION_PARCIAL_DEFAULT;
    if (r3.getSelectedButton() === ui.Button.OK) {
      var parsed = parsearFraccionParcial_(r3.getResponseText());
      if (parsed) fracc = parsed;
    }
    return armarCfgModeloParcial_(elegidos, fracc);
  } catch (errParcial) {
    return off;
  }
}
...
function generarPlanificacionSemanal_() {
  ...
  limpiarFiltros();
  var cfg = leerConfig_();
  cfg.semanas = Math.max(cfg.semanas, SEMANAS_DEFAULT);
  ...
  var nombresHojasSemanas = nombresHojasSemanas_();
  ...
  cfg.modeloParcial = modeloParcialOff_();
  ...
  var txtApoyoToast = (cfg.apoyoL1 && cfg.apoyoL1.activo)
    ...
    : "";
  var txtParcialToast = (cfg.modeloParcial && cfg.modeloParcial.activo)
    ? (", paralelo L1-4 " + textoModeloParcial_(cfg.modeloParcial))
    : "";
  ss.toast("Asignando capacidad (" + cfg.semanas + " semanas, cambio secuencial, L5 hasta 2" + txtApoyoToast + txtParcialToast + ")...", "⚙️ Planificando", 5);
  ...
  tareas.forEach(function (t) {
    if (!mapaModelos[t.modelo]) {
      mapaModelos[t.modelo] = {
        ...
        secuenciaNo: secuenciaNoDeModelo_(mapaSecuencia, t.modelo)
      };
  ...
  cfg.modeloParcial = preguntarModeloParcial_(listaModelos);
  if (cfg.modeloParcial && cfg.modeloParcial.activo) {
    ss.toast("Paralelo L1-4: " + textoModeloParcial_(cfg.modeloParcial), "⚙️ Planificando", 5);
  }
  ...
  function capLinea_(lin) {
    return capFallbackLinea_(lin);
  }

  function capModelo_(m, lin) {
    if (!m) return capFallbackLinea_(lin);
    var cap = 0;
    m.tareas.forEach(function (t) {
      if (t.restante > 0 && Number(t.cap) > cap) cap = Number(t.cap);
    });
    if (cap > 0) return cap;
    m.tareas.forEach(function (t) {
      if (Number(t.cap) > cap) cap = Number(t.cap);
    });
    return cap > 0 ? cap : capFallbackLinea_(lin || (m.tareas[0] && m.tareas[0].lineas[0]));
  }

  function familiaModelo_(m) {
    if (!m) return "";
  ...
  function esModeloParcial_(m) {
    if (!cfg.modeloParcial || !cfg.modeloParcial.activo || !m) return false;
    var noms = nombresCfgModeloParcial_(cfg.modeloParcial);
    var claves = cfg.modeloParcial.claves || [];
    var iP;
    for (iP = 0; iP < noms.length; iP++) {
      if (coincideModeloParcialNombre_(m, noms[iP], claves[iP])) return true;
    }
    return false;
  }
  ...
  function maxOcupantes_(lin, mCand) {
    ...
    if (esLineaEstandar_(lin) && (esModeloParcial_(mCand) || lineaConParcial_(lin))) {
  ...
  var lineaPorMO = {};
  var ultimoModeloLinea = { "1": "", "2": "", "3": "", "4": "", "5": "" };

  function estimarDiaFinLinea_(t, lin, fromDay) {
    var rest = t.restante;
    var capLin = capDeTarea_(t, lin);
  ...
  function capRestanteSemana_(lin, d, capRef) {
  ...
  function producirModeloDia_(mP, lin, d, overflow, techoCarga) {
    var techo = (techoCarga > 0 && techoCarga < 1) ? techoCarga : 1;
  ...
  function producirParaleloEstandar_(noms, lin, d, overflow) {
    var fraccion = (cfg.modeloParcial && cfg.modeloParcial.fraccion > 0)
      ? cfg.modeloParcial.fraccion
      : FRACCION_PARCIAL_DEFAULT;
    if (fraccion < 0.1 || fraccion > 0.9) fraccion = FRACCION_PARCIAL_DEFAULT;
    var ordered = [];
    var iO, jO;
    var nomsCfg = nombresCfgModeloParcial_(cfg.modeloParcial);
    var clavesCfg = (cfg.modeloParcial && cfg.modeloParcial.claves) || [];
    for (jO = 0; jO < nomsCfg.length; jO++) {
      for (iO = 0; iO < noms.length; iO++) {
        if (ordered.indexOf(noms[iO]) !== -1) continue;
        if (coincideModeloParcialNombre_(mapaModelos[noms[iO]], nomsCfg[jO], clavesCfg[jO])) {
          ordered.push(noms[iO]);
        }
      }
    }
    for (iO = 0; iO < noms.length; iO++) {
      if (esModeloParcial_(mapaModelos[noms[iO]]) && ordered.indexOf(noms[iO]) === -1) {
        ordered.push(noms[iO]);
      }
    }
    for (iO = 0; iO < noms.length; iO++) {
      if (ordered.indexOf(noms[iO]) === -1) ordered.push(noms[iO]);
    }
    if (ordered.length < 2) {
      if (ordered.length) producirModeloDia_(mapaModelos[ordered[0]], lin, d, overflow);
      return;
    }
    var fam0 = familiaModelo_(mapaModelos[ordered[0]]);
    var nomB = null;
    for (iO = 1; iO < ordered.length; iO++) {
      if (familiaModelo_(mapaModelos[ordered[iO]]) !== fam0) {
        nomB = ordered[iO];
        break;
      }
    }
    if (!nomB) {
      producirModeloDia_(mapaModelos[ordered[0]], lin, d, overflow);
      return;
    }
    var mA = mapaModelos[ordered[0]];
    var mB = mapaModelos[nomB];
    producirModeloDia_(mA, lin, d, overflow, fraccion);
    producirModeloDia_(mB, lin, d, overflow, 1);
    if (carga[lin][d] < 0.999) producirModeloDia_(mA, lin, d, overflow, 1);
    if (carga[lin][d] < 0.999) producirModeloDia_(mB, lin, d, overflow, 1);
  }

  var ocupante = { "1": [], "2": [], "3": [], "4": [], "5": [] };

  function lineasModelo_(m, overflow) {
    var seen = {};
    var out = [];
    m.tareas.forEach(function (t) {
      if (t.restante <= 0) return;
      elegiblesTarea_(t, overflow).forEach(function (lin) {
        if (seen[lin]) return;
        seen[lin] = true;
        out.push(lin);
      });
  ...
  function lineasClaveModelo_(m, overflow) {
    return lineasModelo_(m, overflow).join(",");
  }

  function compartenLineas_(a, b, overflow) {
    if (!a || !b) return false;
    var sa = lineasModelo_(a, overflow);
    var sb = lineasModelo_(b, overflow);
    for (var iS = 0; iS < sa.length; iS++) {
      if (sb.indexOf(sa[iS]) !== -1) return true;
    }
    return false;
  }
  ...
  function loteFamiliaActivo_(fam, overflow) {
    ...
    for (var iL = 0; iL < listaModelos.length; iL++) {
      ...
      for (var tL = 0; tL < mL.tareas.length; tL++) {
        ...
        if (!best || rank < best.colorRank || (rank === best.colorRank && g < best.genero) ||
            (rank === best.colorRank && g === best.genero && mL.nombre < best.modelo)) {
          best = { modelo: mL.nombre, m: mL, colorRank: rank, genero: g };
        }
      }
    }
    return best;
  ...
  function hayGeneroAnteriorMismaLinea_(m, overflow, dHoy) {
    return debeEsperarLoteFamilia_(m, overflow);
  }
  ...
  function familiaOcupaLinea_(fam, lin, exceptNom) {
    ...
    return (ocupante[lin] || []).some(function (nom) {
      ...
      return mO && familiaModelo_(mO) === fam;
    });
  }

  function lineasDondeEsta_(nombre) {
    return ["1", "2", "3", "4", "5"].filter(function (lin) {
      return ocupante[lin].indexOf(nombre) !== -1;
    });
  }
  ...
  function debeEsperarHermano_(m, overflow) {
    return debeEsperarLoteFamilia_(m, overflow);
  }

  function hermanosPendientesEnLinea_(lin, d, overflow, skip, fam) {
    if (!fam) return [];
    var out = [];
    listaModelos.forEach(function (mC) {
      ...
      var yaOtra = lineasDondeEsta_(mC.nombre).some(function (l2) { return l2 !== lin; });
      if (yaOtra) return;
      if (!modeloPuedeProducirHoyNom_(mC.nombre, lin, d, overflow)) return;
      out.push(mC);
    });
    out.sort(function (a, b) {
      var ga = ordenGenero_(a.genero || generoDeTarea_(a.tareas[0]));
      var gb = ordenGenero_(b.genero || generoDeTarea_(b.tareas[0]));
  ...
  function modeloPuedeProducirHoyNom_(nom, lin, d, overflow) {
    var mO = mapaModelos[nom];
  ...
  for (var d = 0; d < totalDias; d++) {
    ...
    ["1", "2", "3", "4", "5"].forEach(function (lin) {
      ocupante[lin] = ocupante[lin].filter(function (mod) {
        ...
        if (!mO || restanteModelo_(mO) <= 0) return false;
        if (debeEsperarLoteFamilia_(mO, overflowL1)) return false;
        if (debeCederAlLoteFamilia_(mO, overflowL1)) return false;
        return mO.tareas.some(function (t) {
          if (t.restante <= 0 || d < diaInicioEfectivo_(t)) return false;
          if (d < DIAS_LABORALES && (d % DIAS_LABORALES) === t.diaNoLaborable) return false;
    ...
    function desalojarPara_(m, lin) {
      if (String(lin) === "5" && maxOcupantes_(lin, m) > 1) return false;
      var occ = ocupante[lin] || [];
    ...
    ["1", "2", "3", "4", "5"].forEach(function (lin) {
      ...
      if (lastM && restanteModelo_(lastM) > 0 && modeloPuedeProducirHoyNom_(lastNom, lin, d, overflowL1) &&
    ...
    vivos.forEach(function (m) {
      ...
      if (libres.length === 0) {
        libres = lineasDondePuedeHoy_(m).filter(function (lin) {
          return String(lin) !== "5" || maxOcupantes_(lin, m) === 1;
        });
    ...
    function modeloPuedeProducirHoy_(nom, lin, d, overflow) {
      return modeloPuedeProducirHoyNom_(nom, lin, d, overflow);
    }
    ...
    function siguienteCandidato_(lin, d, overflow, skip) {
      ...
      if (famRef && !paraParalelo) {
        ...
        if (loteSig && loteSig.modelo !== lastNom && !skip[loteSig.modelo] &&
            ...
            lineasDondeEsta_(loteSig.modelo).length === 0 &&
            modeloPuedeProducirHoy_(loteSig.modelo, lin, d, overflow) &&
            ocupante[lin].indexOf(loteSig.modelo) === -1) {
          return loteSig.modelo;
        }
      }
      ...
      if (famRef && !paraParalelo && generoTerminado) {
        ...
        if (hermanos.length > 0) return hermanos[0].nombre;
      }
    ...
    function producirLineaDia_(lin, d) {
      ...
      while (carga[lin][d] < 0.999 && guard < 40) {
        ...
        if (esLineaEstandar_(lin) && lineaConParcial_(lin) &&
          ...
          var nextPre = siguienteCandidato_(lin, d, overflow, skip);
          if (nextPre) ocupante[lin].push(nextPre);
        }
        ...
        if (String(lin) === "5" && noms.length >= 2) {
          ...
          var paralelos = noms.filter(function (nom, idx) {
            if (idx === 0) return true;
            return familiaModelo_(mapaModelos[nom]) !== fam0;
          });
          if (paralelos.length >= 2) producirRuedaLinea5_(paralelos, d, overflow);
          else producirModeloDia_(mapaModelos[noms[0]], lin, d, overflow);
        } else if (esLineaEstandar_(lin) && noms.length >= 2 && lineaConParcial_(lin)) {
          producirParaleloEstandar_(noms, lin, d, overflow);
  ...
  for (var linW in hojasLineas) {
    ...
    if (arrData.length > 0) {
      arrData.sort(function (a, b) {
        return cmpFilasFlujoTablero_(a, b, 4, 8, 2);
      });
...
function dibujarTablerosSemanales_(ss, cfg, nombresHojasSemanas, mapaGlobal, infoModelo, hojasLineas, hojaPendiente, jerarquiaInversa) {
  for (var ws = 0; ws < nombresHojasSemanas.length && ws < cfg.semanas; ws++) {
    ...
    if (datosGlobalesS.length > 0) {
      datosGlobalesS.sort(function (a, b) {
        ...
        return cmpFilasFlujoTablero_(a, b, 4, 8, 1);
      });
...
function dibujarAlmacen_(ss, cfg, tareas, infoModelo, totalDias) {
  ...
  listAlmacenModelo.forEach(function (item) {
    ...
    var msEntrada = addBusinessDays_(msSalida, DIAS_ENTRADA_ALMACEN);
...
function actualizarMOs_() {
  ...
  if (cantTerminados > 0) {
    ...
    mensaje += "✅ CULMINADOS/CANCELADOS:\nSe archivaron " + (elimReg + elimEsp) +
      ...
      "Las MO en Hecho de 'Por Hacer - Especial' se conservan.\n\n";
  }
...
function sincronizarProduccionExterna() {
  ...
  function inyectarCantidades(hojaDestino, nombreHoja) {
    ...
    if (idxMoH === -1 || idxProdH === -1) {
      SpreadsheetApp.getUi().alert("⚠️ Aviso: No se pudieron actualizar las cantidades en '" + nombreHoja + "' por falta de encabezados exactos (MO / Cantida Producida).");
      return;
    }
    ...
    for (var r = 0; r < datosH.length; r++) {
      ...
      var valSol = idxSolH !== -1 ? String(datosH[r][idxSolH]).trim() : "";
      ...
      if (rowMoStr !== "") {
        rowMoStr.split(",").forEach(function (mm) {
          var claveMo = claveLookupMO_(mm);
          if (!claveMo) return;
          prodNuevo += (deltaPorMo[claveMo] || 0);
        });
      }

      colProduccion.push([aplicarDeltaCantidaProducida_(datosH[r][idxProdH], prodNuevo)]);
    }
    ...
    for (var r = 0; r < datosLinea.length; r++) {
      ...
      if (rowMoStrL !== "") {
        rowMoStrL.split(",").forEach(function (mm) {
          var claveMoL = claveLookupMO_(mm);
          if (!claveMoL) return;
          produccionEnEstaLinea += (totalesPorLineaYMo[prefijoLinea + "_" + claveMoL] || 0);
        });
      }
    ...
    if (detHeadV.fila !== -1) {
      ...
      if (idMoV !== -1 && idCantV !== -1) {
        for (var iv = detHeadV.fila + 1; iv < datosV.length; iv++) {
          var moV = claveLookupMO_(datosV[iv][idMoV]);
          var cantV = parseCantidad_(datosV[iv][idCantV]);
          if (cantV > 0 && moV !== "") {
  ...
  if (hojaAlmacenSku && hojaAlmacenSku.getLastRow() >= 3) {
    ...
    for (var rS = 0; rS < datosS.length; rS++) {
      ...
      if (rowMoStr !== "") {
        rowMoStr.split(",").forEach(function (mm) {
          var claveAlm = claveLookupMO_(mm);
          if (claveAlm) recHoy += (totalesAlmacenMo[claveAlm] || 0);
        });
      }
...
/** SKU: Ya producida / Produccion Parcial / blanco. Sin producción no hay estado. */
function estadoProdAlm_(producida, faltante, solicitada) {
  var prod = Number(producida) || 0;
  if (!(prod > 0)) return "";
  if (faltante !== "" && faltante !== null && faltante !== undefined) {
    var falt = Number(faltante);
    if (!isNaN(falt)) return falt > 0 ? "Produccion Parcial" : "Ya producida";
  }
  var sol = Number(solicitada) || 0;
  if (sol > 0 && prod >= sol) return "Ya producida";
  return "Produccion Parcial";
}

function aplicarEstadoSkuAlm_(s, producida, faltante, solicitada) {
  s.producida = Number(producida) || 0;
  s.estado = estadoProdAlm_(s.producida, faltante, solicitada != null ? solicitada : s.cantidad);
  s.yaProducida = s.estado === "Ya producida";
  if (s.estado !== "Ya producida" && String(s.entradaAlmacen || "") === "Ya producida") {
    s.entradaAlmacen = "";
  }
  return s.estado;
}

function estadoModeloAlm_(skus, backlog, cantidadModelo, producidaModelo) {
  var rows = (backlog && backlog.length) ? backlog : (skus || []);
  if (!rows.length) {
    return (Number(producidaModelo) || 0) > 0 ? "Produccion Parcial" : "";
  }
  var nYa = 0, nPar = 0, sumCant = 0, i, r, est, prod;
  for (i = 0; i < rows.length; i++) {
    r = rows[i];
    prod = Number(r.producida) || 0;
    est = r.estado || estadoProdAlm_(
      prod,
      r.faltante != null ? r.faltante : r.faltantes,
      r.solicitada != null ? r.solicitada : r.cantidad
    );
    if (est === "Ya producida") nYa++;
    else if (est === "Produccion Parcial") nPar++;
    sumCant += Number(r.solicitada != null ? r.solicitada : r.cantidad) || 0;
  }
  var desglosado = true;
  if (Number(cantidadModelo) > 0 && sumCant + 0.001 < Number(cantidadModelo)) desglosado = false;
  if (desglosado && nYa === rows.length) return "Ya producida";
  if (nYa + nPar > 0 || (Number(producidaModelo) || 0) > 0) return "Produccion Parcial";
  return "";
}
...
function consolidarAlmacenModelo_(resp) {
  ...
  (resp.almacenSku || []).forEach(function (s) {
    ...
    if (!skusPorMod[s.modelo]) skusPorMod[s.modelo] = [];
    skusPorMod[s.modelo].push(s);
  ...
  (resp.backlog || []).forEach(function (b) {
    if (!b || !b.modelo) return;
    if (!backPorMod[b.modelo]) backPorMod[b.modelo] = [];
    backPorMod[b.modelo].push(b);
  });
  Object.keys(byMod).forEach(function (nom) {
    var m = byMod[nom];
    m.estado = estadoModeloAlm_(
      skusPorMod[nom] || [],
      backPorMod[nom] || [],
      m.cantidad,
      m.producida
    );
    m.yaProducida = m.estado === "Ya producida";
    if (m.estado !== "Ya producida" && String(m.entradaAlmacen || "") === "Ya producida") {
      m.entradaAlmacen = "";
    }
  });
}
...
function aplicarProducidaAlmacen_(ss, resp) {
  ...
  (resp.almacenSku || []).forEach(function (s) {
    ...
    if (ord.cerradas[key]) {
      s.cerrada = true;
      aplicarEstadoSkuAlm_(s, 0, 1, s.cantidad);
      return;
    }
    ...
    if (rec) {
      ...
      s.faltantes = rec.faltante > 0 ? rec.faltante : 0;
      aplicarEstadoSkuAlm_(s, rec.producida || 0, rec.faltante, rec.solicitada || s.cantidad);
      if (s.yaProducida && !s.entradaAlmacen) s.entradaAlmacen = "Ya producida";
      return;
    }
    if (ord.abiertas.hasOwnProperty(key)) {
      s.cerrada = false;
      aplicarEstadoSkuAlm_(s, ord.abiertas[key] || 0, null, s.cantidad);
      if (s.yaProducida && !s.entradaAlmacen) s.entradaAlmacen = "Ya producida";
      return;
    }
    if (ord.skuAbierta.hasOwnProperty(sku)) {
      s.cerrada = false;
      aplicarEstadoSkuAlm_(s, ord.skuAbierta[sku] || 0, null, s.cantidad);
      if (s.yaProducida && !s.entradaAlmacen) s.entradaAlmacen = "Ya producida";
      return;
    }
    if (ord.skuVista[sku]) {
  ...
  function inyectarRec_(rec) {
    ...
    var est = estadoProdAlm_(rec.producida, rec.faltante, rec.solicitada);
    var ya = est === "Ya producida";
...
function obtenerDatosDashboardCompleto() {
  ...
  var nombresSemanas = nombresHojasSemanas_();
