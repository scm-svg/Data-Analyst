# Planificación de Producción v5.9.11 — códigos listos para pegar
...
- **Sincronizar Producción:** solo escribe `Cantida Producida` (Por Hacer col. L / Especial col. M). No pisa `Faltante`. **Por Hacer es la línea base** de lo ya producido. La primera corrida de v5.9.11 **no suma** el histórico de `Produccion - Costura` (solo guarda el corte). Después suma **solo lo nuevo**: `existente + max(0, costura_ahora − ya_aplicado)`. Si v5.9.9/5.9.10 ya había sumado el total (cantidades duplicadas), esta corrida **deshace esa suma** y deja el incremento real. Las MO se cruzan sin ceros a la izquierda (`00071` = `71`).
