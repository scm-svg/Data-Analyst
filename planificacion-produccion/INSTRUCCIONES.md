# Planificación de Producción v5.9.10 — códigos listos para pegar
...
- **Sincronizar Producción:** solo escribe `Cantida Producida` (Por Hacer col. L / Especial col. M). No pisa `Faltante`. Suma Costura **sobre lo ya cargado**: `existente + max(0, costura_ahora − ya_aplicado)`. La primera corrida (o una línea base vieja de v5.9.8, que se ignora) suma el total de Costura. Después solo el incremento. Las MO se cruzan sin ceros a la izquierda (`00071` = `71`).
