# Planificación de Producción v5.9.30 — códigos listos para pegar
...
- No reclama la segunda línea ni parte una MO, **salvo** Urgente/mínima en la ventana de fecha: ahí sí toma la segunda línea **si está libre**. Vacío u otro valor: secuencia normal de 5.9.15.
- **Urgente / mínima con 2+ líneas** (ej. COTTON KIDS en `2, 4`): el día de **Fecha de Salida Estimada** y el hábil anterior reparte el faltante en las asignadas **libres**. No desaloja a quien ya ocupa la línea. `Secuencia=No` no impide tomar esa segunda línea libre.
...
## Motor v5.9.30 (base 5.9.15 + Secuencia=No + Urgente en fecha)

- **Urgente cerca de fecha estimada:** un modelo Urgente o con cantidad mínima que lista 2+ líneas (COTTON KIDS en `2, 4`) **no** explota el día que entra. El **día de Fecha de Salida Estimada** y el **hábil anterior** reparte el faltante en las asignadas **libres**. No desaloja a RIO ni a otro ocupante. Sin fecha no explota; si la fecha ya venció al inicio del horizonte, explota desde el día 0. **Fuera de esa ventana**, la segunda línea solo entra si está libre y el modelo no tiene `Secuencia=No`. En la ventana no espera lote de color (mezcla para llenar) y `Secuencia=No` **sí** permite la segunda línea libre. La familia sigue compartiendo un género por línea. Media/Alta/Baja conservan Negro → Blanco → Marino y CAB → DAMA → KIDS.
...
- **Urgente** después de Especial y de la cantidad mínima, luego la fecha de salida más próxima. Con 2+ líneas, el día de la fecha estimada (y el hábil anterior) usa las asignadas **libres**; no echa al ocupante. `Secuencia=No` no bloquea esa toma. Si un hermano de la misma familia todavía no tiene línea, no se la quita.
