#!/usr/bin/env python3
"""Generate embedded DATA JSON for sportlite.html from Excel."""
import json
import re
...
XLSX = '/home/ubuntu/.cursor/projects/workspace/uploads/SPORTLITE_COMPLETO_e034.xlsx'
MESES = [
...
def norm_genero(g):
    g = str(g).strip().upper()
    return 'CAB' if g in ('CABALLERO', 'CAB') else 'DAMA' if g == 'DAMA' else g


def norm_modelo(m):
    m = str(m).strip().upper()
    if 'MIKA' in m:
        return 'MIKA SPORT LITE'
    if 'NOAH' in m:
        return 'NOAH SPORT LITE'
    if 'MAYA' in m:
        return 'MAYA SPORT LITE'
    return str(m).strip()
...
def loc_key(name):
    t = norm_tienda(name)
    inv_map = {'CHACAO': 'SAMBIL CHACAO', 'SAMBIL': 'SAMBIL VALENCIA', 'GRIETA': 'GRIE'}
...
def read_ventas():
    ...
    for _, row in df.iterrows():
        ...
        if qty == 0:
            continue
        fecha = row['FECHA'] if pd.notna(row.get('FECHA')) else row['Fecha de la orden']
        rows.append({
...
def compute_prod_curve(raw_rows, stock, stock_by_loc):
    ...
    for modelo, genero, color, talla in sorted(keys):
        ...
        need = lambda n: max(0, round(v_mes * n - stk_total))
        curve.append({
            'modelo': modelo,
            'genero': genero,
            'talla': talla,
...
def compute_margarita(raw_rows, prod_curve, mult=2.0):
    base = [m for m in BASE_MESES if any(r['mes'] == m for r in raw_rows)]
    if not base:
        base = sorted({r['mes'] for r in raw_rows}, key=mes_sort_key)[-3:]
    ...
    for (modelo, genero, color, talla), v3m in sorted(grie_sales.items()):
        ...
        if v_mes <= 0:
            continue
        need = lambda n: max(0, round(v_mes * n))
        skus.append({
            'MODELO': modelo,
            'GENERO': genero,
            'COLOR': color,
...
def build():
    ...
    margarita = compute_margarita(raw_rows, prod_curve, mult=2.0)
