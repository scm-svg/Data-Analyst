#!/usr/bin/env python3
"""Generate embedded DATA JSON for sportmesh.html from Excel."""
import json
from collections import defaultdict

import pandas as pd

XLSX = '/home/ubuntu/.cursor/projects/workspace/uploads/SPORT_MESH_COMPLETO_090b.xlsx'
MESES = [
...
def norm_modelo(m):
    ...
    if 'ELIMINAR' in m:
        return None
    if 'CLASICA' in m or 'CLÁSICA' in str(m).upper():
...
def read_ventas():
    ...
    for _, row in df.iterrows():
        ...
        if not mes:
            continue
        rows.append({
            'tienda': norm_tienda(row['UBICACIÓN']),
...
if __name__ == '__main__':
    ...
    print(f'Sales rows: {len(data["raw_rows"])}, units: {data["total"]}', file=__import__('sys').stderr)
    print(f'Stock: {data["stock_total"]}, models: {data["filtros"]["modelos"]}', file=__import__('sys').stderr)
    print(f'Range: {data["date_range"]}', file=__import__('sys').stderr)
